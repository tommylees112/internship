...
def predict_update_chart(box_bg="#CCCCCC", arrow1="#88CCFF", arrow2="#88FF88"):
...
def show_residual_chart(show_eq=True, show_H=False):
    plt.figure(figsize=(11, 3.0), facecolor="w")
    est_y = (164.2 - 158) * 0.8 + 158
    ...
    ax.annotate(
        ...
        xy=[1, 159],
        xytext=[0, 158],
        arrowprops=dict(arrowstyle="->", ec="r", lw=3, shrinkA=6, shrinkB=5),
    )
    ...
    ax.annotate(
        ...
        xy=[1, 159],
        xytext=[1, 164.2],
        arrowprops=dict(arrowstyle="-", ec="k", lw=3, shrinkA=8, shrinkB=8),
    )
    ...
    ax.annotate(
        ...
        xy=(1.0, est_y),
        xytext=(0.9, est_y),
        arrowprops=dict(arrowstyle="->", ec="#004080", lw=2, shrinkA=3, shrinkB=4),
    )
    ...
    if show_eq:
        plt.text(
            0.75, est_y - 1.2, r"$\bar{x}_t + Ky$", ha="right", va="center", fontsize=18
        )
    plt.xlabel("time")
...
def plot_predictions(p, rng=None, label="Prediction"):
    if rng is None:
        rng = range(len(p))
    plt.scatter(
        rng, p, marker="v", s=40, edgecolor="r", facecolor="None", lw=0.5, label=label
    )
...
def plot_measurements(
    xs, ys=None, dt=None, color="k", lw=1, label="Measurements", lines=False, **kwargs
):
...
def plot_residual_limits(Ps, stds=1.0):
